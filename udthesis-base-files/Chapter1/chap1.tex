...
\label{alg2}
\begin{algorithmic}
\IF{$\nu$ is a leaf and $f$ cannot be resolved by interpolant($\nu$)}
\STATE Define new nodes $\nu_0$, $\nu_1$.
\STATE $[a,b]$:=domain($\nu$)
\STATE $m = \frac{a+b}{2}$
\STATE zone(\child{0}($\nu$)) = $[b,m]$
\STATE zone(\child{1}($\nu$)) = $[m,a]$
\STATE domain(\child{0}($\nu$)), domain(\child{1}($\nu$)) are formed as in (\ref{eq:zone_extend1d})
\STATE domain($\nu$) = domain(\child{0}($\nu$)) $\cup$ domain(\child{1}($\nu$))
\STATE \child{0}($\nu$) := $\nu_0$
\STATE \child{1}($\nu$) := $\nu_1$
\STATE \weight{0}($\nu$),\weight{1}($\nu$):= weights in (\ref{PUW}) defined for domain($\nu_0$), domain($\nu_1$)
...
\item leafpoints($\nu$):= provides the Chebyshev points of $T$ in domain($\nu$) i.e. \newline $\text{points(root($T$))} \cap \text{domain($\nu$)}$
