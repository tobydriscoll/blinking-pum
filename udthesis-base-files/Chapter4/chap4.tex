...
\end{cases}
\end{equation}
where $h_e$ is the thickness of tear fluid under eyelid \cite{heryudono2007single}, and $Q_{lg}$ is the influx from the lacrimal gland \cite{braun2015dynamics}. For the outflux function we have
\begin{equation}
...
For our first experiment, we choose $\pa$ and $\ps$ to be $6.11 \times 10^{-6}$ and $3.09 \times 10^{-6}$, values chosen from \cite{braun2015dynamics}. The thickness of the tear film fluid $h_e$ is set to 2. For simplicity we set  $Q_{lg}$ and $Q_p$ to zero. We solve the model with our preconditioned method, using the partitioning in Figure~\ref{eye_partition} to better adapt to the features of the tear film height. For each subdomain we use a $35 \times 35$ Chebyshev interpolant approximation.
\begin{figure}
	\centering
...
We first examine the case of a stationary domain. The boundary conditions of \ref{the_pde} ensure that the volume of the fluid $h(x,y,t)$ is constant over time (in the case of a stationary boundary). As seen in Figure~\ref{eye_volume}, our method maintains a volume that is relatively within $10^{-6}$ of the initial volume, which is indicative of the pointwise error of the numerical solution.

We now solve the same model but with a moving boundary prescribed by the realistic blink motion of \cite{deng2014heat} where the eye closes 20 percent. With the moving boundary, we expect the volume to be conserved over a blink cycle (that is, it is conserved periodically). After one blink cycle the volume is conserved within a relative difference of 0.01.


\begin{figure}
	\centering
	\includegraphics[scale=0.8]{Chapter4/stationary_volume_change}
	\caption{Change of volume of the fluid over time. The boundary conditions ensure constant volume over time.}
	\label{eye_volume}
\end{figure}
