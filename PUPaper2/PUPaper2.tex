...
\begin{algorithm}
\caption{sample($\Omega$,$\nu$,$f$)}
\label{alg:LSsample}
\begin{algorithmic}
\STATE define a grid $F$ of Chebyshev 2nd-kind points on \textsf{domain}($\nu$) twice as fine as \textsf{grid}($\nu$) in each dimension.
\STATE define a collocation matrix $M$ that maps \textsf{coefficients}($\nu$) to $\text{\textsf{approximation}($\nu$)}\rvert_{F \cap \Omega}$.
\STATE define \textsf{coefficients}($\nu$) to be a set of coefficients $C$ that minimize $\| MC - f \rvert_{F \cap \Omega} \|^2$.
\end{algorithmic}
\end{algorithm}
