...
\end{equation}
where $\Xi = \{\vect{x}_i\}_{i=1}^{P} \subset \Omega_\nu\cap \Omega$ is a point set in the ``active'' part of the leaf's domain, $\Omega_\nu\cap \Omega$. In practice we can form a matrix $A$ whose columns are evaluations of each basis function at the points in $\Xi$, leading to a standard $P\times N^d$ linear least squares problem. We choose $\Xi$ as the active part of the standard $(2N)^d$-sized Chebyshev grid. 

This technique resembles Fourier extension or continuation techniques~\cite{adcock2014resolution,adcock2014numerical,huybrechs2010fourier}, so we refer to it as a \emph{Chebyshev extension approximation}. Unlike the Fourier case, however, there is no real domain extension involved; rather one contrains the usual multivariate polynomial only over part of its most natural domain. The condition number of $A$ in the discrete Fourier extension case has been shown to increase exponentially with the degree of the approximation \cite{adcock2014numerical}, but we presumably can avoid having problems with this by capping $N$ and subdividing the domain instead.

REWORK THESE PARAGRAPHS/ALGORITHM

We also make four modifications to the refinement criteria of Algorithm~\ref{alg:splitleaves}. First, we add a geometric refinement rule: if $\Omega_\nu\cap\Omega$ is nonempty but the centroid of $\Omega_\nu$ lies outside $\Omega$, the leaf is refined. Second, following a split the extents of the new domains in the split dimension are narrowed to the smallest intervals needed to contain $\Omega$. These rules help to ensure that a substantial fraction of each leaf's domain lies inside $\Omega$.

The third modification has to do with determining whether the approximation on a nonrectangular leaf is of sufficient accuracy. Here we use the residual of the least-squares solution (normalized by $\sqrt{P}$) as a proxy for the true error. The final modification of Algorithm~\ref{alg:splitleaves} is that if a leaf needs to be refined, all dimensions are flagged for refinement. Our experiments indicate that even for a function on $\Omega$ (such as $xT_{60}(y)$) that has dramatically different degree requirements in its different variables, the extension of this function to the total domain of the leaf, as computed by the least squares procedure, has homogeneous dependence on the degree in all variables. 
...
\begin{table}
  \begin{tabular}{c|c|c|c|c|c}
    ...
    \multirow{3}{*}{$g_1$} & disk & 5.44E-15 & 0.103 & 0.002 & 289 \\
             & diamond &  2.11E-11 & 0.029 & 0.001 & 289 \\
             & astroid &  5.18E-10 & 0.045 & 0.001 & 289 \\\hline
    \multirow{3}{*}{$g_2$} & disk & 2.40E-10 & 0.798 & 0.031 & 3757 \\
             & diamond &  1.34E-09 & 0.528 & 0.016 & 2890 \\
             & astroid & 1.28E-09 & 1.453 & 0.022 & 4624 \\ \hline
    \multirow{3}{*}{$g_3$} & disk & 4.44E-11 & 10.327 & 1.075 & 245650 \\
             & diamond & 1.79E-11 & 10.123 & 0.675 & 165310 \\
             & astroid & 7.90E-10 & 19.583 & 0.581 & 140450 \\ \hline 
    \multirow{3}{*}{$g_4$} & disk & 7.49E-11 & 1.470 & 0.077 & 12138 \\
             & diamond & 9.65E-12 & 1.562 & 0.048 & 9826 \\
             & astroid & 1.01E-10 & 2.981 & 0.048 & 9826 
  \end{tabular}
