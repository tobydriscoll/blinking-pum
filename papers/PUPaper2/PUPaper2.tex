...
\end{equation}
We have $\sum_{\nu\in \text{leaves}(\ct)} w_\nu(\vect{x}) = 1$ which makes $\{w_\nu (\vect{x})\}$ a partition of unity. This implies that for a patch $\nu$ that for all $\vect{x}$ in $\nu$ that lie in no other patches that $w_\nu(\vect{x})~=~1$. Thus if we assume that weights only have support in their respective domains, smoothness of the partition of unity functions is only possible as long as there is overlap between neighboring patches.
...
\begin{equation}
  ...
  \label{eq:pu-approx}	
\end{equation}
Despite being composed of separate local approximations from a partitioned domain, the global approximation (\ref{eq:pu-approx}) remains infinitely smooth all while avoiding global restraints such as matching. Given the smoothness of the partition of unity $\{w_\nu (\vect{x})\}$, (\ref{eq:pu-approx}) can approximate $f$ and its derivatives \cite{wendland2004scattered}.
...
\begin{table}[p]
  \begin{tabular}{c|c|c|c|c|c}
    \multirow{2}{*}{Function} & \multirow{2}{*}{Alg.} & \multirow{2}{*}{Error} & Build & Eval & Points /  \\
 & &  & time & time &Rank  \\ \hline
    \multirow{2}{*}{$\cos(u_1\pi + \sum_{i=1}^3 a_i x_i)$} & T & 3.00$\times 10^{-14}$ &	0.760 &	0.121 &	2197000 \Tstrut\\
    & C & 2.16$\times 10^{-14}$ & 0.061 & 0.15 & 2  \\ \hline
    \multirow{2}{*}{$\prod_{i=1}^3 (a_i^{-2}+(x_i-u_i)^2)^{-1}$} & T & 4.47$\times 10^{-7}$	 & 17.240 & 0.854 & 22553706 \Tstrut\\
    & C & 5.66$\times 10^{-7}$ & 0.057 & 0.14 & 1 \\ \hline
    \multirow{2}{*}{$(1+\sum_{i=1}^3 a_i x_i)^{-3}$} & T & 2.62$\times 10^{-10}$ &	0.147	& 0.007 & 125 \Tstrut\\
    & C & 4.07$\times 10^{-10}$ & 0.038 & 0.14 & 4 \\ \hline
    \multirow{2}{*}{$\exp(-\sum_{i=1}^2 a_i^2 (x_i-u_i)^2)$} & T & 2.00$\times 10^{-15}$ &	0.688 &	0.103 &	834568 \Tstrut  \\
    & C & 7.77$\times 10^{-16}$ & 0.047 & 0.14 & 1 \\ \hline
    \multirow{2}{*}{$1/(\cosh(5(x+y+z)))^2$} & T & 3.33$\times 10^{-15}$ & 5.882 & 0.387 & 12740192 \Tstrut\\
    & C & 3.69$\times 10^{-13}$ & 81 & 0.40 & 93 \\ \hline
    \multirow{2}{*}{$\arctan(5(x+y)+z)$} & T & 4.88$\times 10^{-15}$ & 2.287 &	0.184 &	3083990 \Tstrut\\
